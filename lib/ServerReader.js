...
function createServers(file, parsedFile, cb) {
  ...
  async.each(parsedFile, function (serverJson, cb) {
    ...
    if (serverJson.pems) {
      ...
      var pemFilePaths = serverJson.pems.map(pem => {
        return path.resolve(filePath, pem);
      })
      auth = new Auth(Auth.PEM, path.resolve(filePath, serverJson.pems[0]), pemFilePaths);
    } else if (serverJson.pem) {
      var filePath = path.normalize(path.parse(file).dir);
    ...
    var server = new Server(serverJson.name, serverJson.user, serverJson.host, port, auth, serverJson.secondhost);
